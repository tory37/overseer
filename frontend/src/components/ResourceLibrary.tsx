...
import { getSkills, createSkill, updateSkill, deleteSkill, getAgents, createAgent, updateAgent, deleteAgent, getBaseUrl } from '../utils/api';
...
export const ResourceLibrary: React.FC<ResourceLibraryProps> = ({ type, onResourcesChanged }) => {
  ...
  const fetchResources = async () => {
    setIsLoading(true);
    try {
      const data = type === 'skill' ? await getSkills() : await getAgents();
      setResources(data);
      
      const configRes = await fetch(`${getBaseUrl()}/api/config`);
      const configData = await configRes.json();
      setIsConfigured(!!(type === 'skill' ? configData.skills_directory : configData.agents_directory));
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const payload = { name, category, description, content };
      if (isNew) {
        const result = type === 'skill' ? await createSkill(payload) : await createAgent(payload);
        setSelectedResourceId(result.id);
        setIsNew(false);
      } else if (selectedResourceId) {
        const result = type === 'skill' 
          ? await updateSkill(selectedResourceId, payload) 
          : await updateAgent(selectedResourceId, payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      if (type === 'skill') {
        await deleteSkill(selectedResourceId);
      } else {
        await deleteAgent(selectedResourceId);
      }
      setSelectedResourceId(null);
      await fetchResources();
      onResourcesChanged?.();
                ...
                {isConfigured === false && (
                  <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl space-y-2">
                    ...
                    <p className="text-[10px] text-slate-400 leading-relaxed">
                      You haven't configured an {typeLabel.toLowerCase()}s directory yet. {typePlural} cannot be saved until you set a storage path in the Configuration tab.
                    </p>
                  </div>
                )}
