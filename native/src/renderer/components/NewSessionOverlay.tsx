...
const CLI_OPTIONS: { id: CliType; label: string; color: string }[] = [
  { id: 'gemini', label: 'Gemini', color: 'blue' },
  { id: 'claude', label: 'Claude', color: 'orange' },
  { id: 'cursor-agent', label: 'Cursor', color: 'purple' },
];
...
export const NewSessionOverlay: React.FC<NewSessionOverlayProps> = ({ repository, onClose, onLaunch }) => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const [loadedPersonas, config, loadedBranches] = await Promise.all([
          ipcRenderer.invoke('store-load-personas'),
          ipcRenderer.invoke('store-load-app-config'),
          ipcRenderer.invoke('git-list-branches', { repoPath: repository.path }),
        ]);
        ...
        <div className="px-8 py-6 border-t border-slate-800 bg-slate-950/50 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
              {repository.name} {isWorktree && `/ Worktree: ${worktreeName || '...'}`}
            </span>
          </div>
          <button
